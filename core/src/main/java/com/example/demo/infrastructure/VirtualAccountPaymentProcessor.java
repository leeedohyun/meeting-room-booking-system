...
@Profile("!test")
public class VirtualAccountPaymentProcessor implements PaymentProcessor {
    ...
    public PaymentResult process(Long reservationId, int amount) {
        log.info("가상계좌 결제 처리: 예약 ID = {}, 금액 = {}", reservationId, amount);
        ...
        Map<String, Object> response = restClient.post()
                .uri("/api/c-corp/payments/virtual-account")
                .body(request)
                .retrieve()
                .body(new ParameterizedTypeReference<>() {});

        return new PaymentResult(
                response.get("status").equals("SUCCESS") ? true : false,
                (String) response.get("message"),
                (String) response.get("va_id"),
                amount,
                LocalDateTime.now()
        );
    }
