...
public class CardPaymentProcessor implements PaymentProcessor {
    ...
    public PaymentResult process(Long reservationId, int amount) {
        log.info("카드 결제 처리: 예약 ID = {}, 금액 = {}", reservationId, amount);
        ...
        Map<String, Object> response = restClient.post()
                .uri("/api/a-corp/payments")
                .body(request)
                .retrieve()
                .body(new ParameterizedTypeReference<>() {});

        return new PaymentResult(
                (int) response.get("status_code") == 200 ? true : false,
                (String) response.get("message"),
                (String) response.get("transaction_id"),
                (Integer) response.get("approved_amount"),
                (LocalDateTime) response.get("approved_at")
        );
    }
}
